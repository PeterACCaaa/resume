...
- 参与半导体行业交流平台微信小程序开发，前端基于 Vue 2 承接页面迭代、表单交互、列表展示、详情页和业务流程优化，配合后端接口完成数据渲染、状态处理和异常提示。
- 后端早期以 PHP 为主，后续参与团队从 PHP 向 Go 技术栈转型过程中的接口开发、业务逻辑调整与前后端联调，协助梳理旧接口行为、参数字段、返回结构和新接口适配差异，保证存量业务路径在迁移过程中可继续使用。
- 参与版本测试、问题修复与上线发布，基于 Git 进行协同开发与版本管理，编写局部测试脚本并配合完成站点部署流程，保证迭代功能按版本节奏交付。
